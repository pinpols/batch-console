...
> 日期:2026-07-03
> 视觉源:`Batch Console 设计定稿.html`
> 设计规格:`export/batch-console-设计规格.md`
> Token 交付:`refs/tokens-handoff.css`

## 1. 定稿口径

- 信息架构采用 **IA v3 / 7 组**:工作台、运行监控、告警与投递、作业与流程、文件、调度治理、系统管理。
- 低频 admin 入口不进侧边栏:配额策略、队列配置、运行窗口、业务日历,通过 Command Palette 可达。
- 本目录是本轮重设计的最新权威入口。`docs/design/` 下的旧方案文档只保留架构背景,不作为视觉定稿依据。
...
- 错误重复目录 `docs/design/redesign/` 已移除。
- IA 冲突口径收敛为 IA v3 / 7 组。

验收:

- `design/README.md` 能说明最新设计入口。
- `docs/README.md` 和 `docs/design/README.md` 能指向根 `design/`。
