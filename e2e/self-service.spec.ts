...
test.describe('self-service panel (自助服务)', () => {
  ...
  test('自助服务页面可打开并展示 4 个标签页', async ({ page }) => {
    ...
    await expect(page.locator('.service-entry').filter({ hasText: '配额与用量' }).first()).toBeVisible()
    await expect(page.locator('.service-entry').filter({ hasText: '配额变更' }).first()).toBeVisible()
    await expect(page.locator('.service-entry').filter({ hasText: '重跑申请' }).first()).toBeVisible()
    await expect(page.locator('.service-entry').filter({ hasText: '补偿申请' }).first()).toBeVisible()
  })
