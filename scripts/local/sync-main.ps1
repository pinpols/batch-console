<#
.SYNOPSIS
  把 origin/main 的更新合并到本地 feature/docker-deploy(前端,Windows / PowerShell 7+)。

.DESCRIPTION
  与 scripts/sync-mainY.sh 行为一致的 PowerShell 版本。
  仅做 git 操作,不触发部署。部署用 scripts/deploy.ps1 up。

  首次同步若两边无共同祖先(本地是快照仓库),会自动加 --allow-unrelated-histories,
  一次性引入 origin/main 全部内容,生成大 merge commit。后续 sync 行为正常。

  合并成功不自动 push;前端 docker compose up 验证通过后再手动 git push。
...
Write-Host ""
Write-Host "  下一步:"
Write-Host "    1) 跑前端部署验证:"
Write-Host "         ./scripts/deploy.ps1 up"
Write-Host "    2) http://localhost:8080 + LAN IP 都能访问后再 push:"
