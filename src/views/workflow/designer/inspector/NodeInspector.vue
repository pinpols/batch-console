...
<template>
  <aside class="node-inspector" :aria-label="t('workflowDesignerMvp.inspectorAriaLabel')">
    ...
    <template v-else>
      <StartNodeForm v-if="selectedNode.nodeType === 'START'" :node="selectedNode" :readonly="readonly" />
      <EndNodeForm v-else-if="selectedNode.nodeType === 'END'" :node="selectedNode" :readonly="readonly" />
      <JobNodeForm v-else-if="selectedNode.nodeType === 'JOB'" :node="selectedNode" :readonly="readonly" />
      <FileStepNodeForm
...
.node-inspector {
  width: 320px;
  border-left: 1px solid var(--color-border-base, #dcdfe6);
