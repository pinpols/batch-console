<script setup lang="ts">
  ...
  defineExpose({
    ...
    /** 将全部节点完整放进当前画布视口。 */
    fitToViewport: () => handle.fitToViewport(),
    /** 校验错误列表点击 → 居中并选中该节点 */
    focusNode: (nodeId: string) => handle.focusNode(nodeId),
    /** 连线校验错误列表点击 → 居中并选中该连线 */
    focusEdge: (edgeId: string) => handle.focusEdge(edgeId),
...
<template>
  ...
  >
    ...
    <div class="dag-canvas__legend" :aria-label="t('workflowDesignerMvp.edgeLegendAria')">
      <span class="dag-canvas__legend-item">
        <i class="dag-canvas__legend-line dag-canvas__legend-line--success" />
        {{ t('enum.edgeType.SUCCESS') }}
      </span>
      <span class="dag-canvas__legend-item">
        <i class="dag-canvas__legend-line dag-canvas__legend-line--failure" />
        {{ t('enum.edgeType.FAILURE') }}
      </span>
      <span class="dag-canvas__legend-item">
        <i class="dag-canvas__legend-line dag-canvas__legend-line--condition" />
        {{ t('enum.edgeType.CONDITION') }}
      </span>
      <span class="dag-canvas__legend-item">
        <i class="dag-canvas__legend-line dag-canvas__legend-line--always" />
        {{ t('enum.edgeType.ALWAYS') }}
      </span>
    </div>
    <div
      ref="minimapRef"
      class="dag-canvas__minimap"
      :aria-label="t('workflowDesignerSpike.minimapAriaLabel')"
    />
    <TeleportContainer />
  </div>
</template>
...
<style scoped>
  ...
  .dag-canvas__legend-item {
    display: inline-flex;
    align-items: center;
    gap: 4px;
    white-space: nowrap;
  }
  .dag-canvas__legend-line {
    display: inline-block;
    width: 18px;
    border-top: 2px solid;
  }
  .dag-canvas__legend-line--success {
    border-color: var(--color-success);
  }
  .dag-canvas__legend-line--failure {
    border-color: var(--color-danger);
    border-top-style: dashed;
  }
  .dag-canvas__legend-line--condition {
    border-color: var(--color-warning);
    border-top-style: dotted;
  }
  .dag-canvas__legend-line--always {
